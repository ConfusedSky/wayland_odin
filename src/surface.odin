...
can_initialize_surface :: proc(state: ^state_t) -> bool {
	return(
		...
		state.dmabuf.proxy.id != 0 &&
		state.dmabuf.feedback_done &&
...
initialize_surface :: proc(state: ^state_t) -> Errno {
	...
	vk_buf, err := renderer.allocate_vulkan_buffer(
		&state.vulkan,
		state.max_w,
		state.max_h,
		state.dmabuf.modifier,
	)
	if err != nil do return err
	state.dmabuf.vk_buf = vk_buf
...
initialize_buffer :: proc(state: ^state_t) -> Errno {
	if state.dmabuf.wl_buf.id != 0 {
		wl_buffer.destroy(&state.dmabuf.wl_buf) or_return
		// Keep the handler registered until the compositor sends on_release
	}
	state.dmabuf.wl_buf = import_as_wl_buffer(
		state,
		&state.dmabuf.vk_buf,
		state.w,
		state.h,
	...
	register_event_handler(
		state,
		state.dmabuf.wl_buf.id,
		&wl_buffer_handlers,
		wl_buffer.handle_event,
	)
	state.dmabuf.buf_w = state.w
	state.dmabuf.buf_h = state.h
...
draw_next_frame :: proc(state: ^state_t) -> Errno {
	...
	assert(state.dmabuf.vk_buf.memory != 0)
	assert(state.w > 0 && state.h > 0)
	assert(state.dmabuf.buffer_ready)

	fmt.printfln("Drawing next frame")

	if state.w != state.dmabuf.buf_w || state.h != state.dmabuf.buf_h {
	...
	wl_surface.attach(&state.wl_surface, &state.dmabuf.wl_buf, 0, 0) or_return
	...
	state.dmabuf.buffer_ready = false
	wl_surface.commit(&state.wl_surface) or_return
